...
export default function Dashboard({ role }: DashboardProps) {
    ...
    // State for Gateway Interaction
    const [isGatewayExpanded, setIsGatewayExpanded] = useState(false);
    const [isGatewayHovered, setIsGatewayHovered] = useState(false);
    const gatewayContainerRef = useRef<HTMLDivElement>(null); // Use specific name to avoid conflict

    // Gateway Scroll Interaction
    useEffect(() => {
        const handleWheel = (e: WheelEvent) => {
            if (!isGatewayHovered || isGatewayExpanded) return;
            if (e.deltaY > 10) {
                e.preventDefault();
        ...
        const element = gatewayContainerRef.current;
        if (element) {
    ...
    }, [isGatewayHovered, isGatewayExpanded]);

    // Group gateways for expanded view
        ...
        <div className="flex h-screen bg-gray-50">
            ...
            <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
                ...
                <main className="flex-1 overflow-auto p-8 relative">
                    ...
                    {/* --- INTEGRATION GATEWAYS SECTION START --- */}
                    <div className="mb-10">
                        {/* Header */}
                        ...
                        <div
                            ...
                            onMouseEnter={() => setIsGatewayHovered(true)}
                            onMouseLeave={() => setIsGatewayHovered(false)}
                            className={`
                                relative transition-all duration-500 ease-out border-2 border-dashed border-gray-200 rounded-[2rem] p-6 bg-white
                                ${isGatewayHovered && !isGatewayExpanded ? 'border-blue-400 bg-blue-50/10 scale-[1.01] shadow-lg cursor-ns-resize z-10' : ''}
                            `}
                        >
                            {/* Hover Hint */}
                            <div className={`
                                ...
                                ${isGatewayHovered && !isGatewayExpanded ? 'opacity-100' : 'opacity-0'}
                            `}>
                    ...
                    {/* --- INTEGRATION GATEWAYS SECTION END --- */}
                ...
                `}>
                    ...
                    <div className={`
                        ...
                        ${isGatewayExpanded ? 'scale-100 translate-y-0' : 'scale-95 translate-y-10'}
                    `}>
                        {/* Header */}
                                ...
                                {CATEGORIES.map(category => (
                                    <div key={category}>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                                            {groupedGateways[category]?.map((gateway: any) => (
                                                <div
                                                    key={gateway.id}
